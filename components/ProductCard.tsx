...
export default function ProductCard({ product, compact = false }: ProductCardProps) {
  ...
  const handleBuy = () => {
    if (product.link === 'YOUR_AFFILIATE_LINK_HERE') {
  ...
  return (
    ...
    >
      ...
      <div className={`flex flex-col gap-1.5 flex-1 ${compact ? 'p-4' : 'p-5'}`}>
        ...
        {!compact && <p className="text-sm text-muted leading-relaxed">{product.desc}</p>}
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <span className="text-sm text-muted line-through">{product.mrp}</span>
        ...
        >
          {feedback || `Check Price on ${product.where} →`}
        </button>
      </div>
    </div>
  )
